AM_DOCKER_IMAGE_NAME_PREFIX ?= ghcr.io/hantdev
BUILD_DIR = build
...
define compile_service
	...
	-X 'github.com/hantdev/certs/http.BuildTime=$(TIME)' \
	-X 'github.com/hantdev/certs/internal/http.Version=$(VERSION)' \
	-X 'github.com/hantdev/certs/internal/http.Commit=$(COMMIT)'" \
...
run:
	docker compose -f docker/docker-compose.yml --env-file docker/.env -p hantdev up

deploy:
	docker compose -f docker/docker-compose.yml pull
	docker compose -f docker/docker-compose.yml --env-file docker/.env -p hantdev up -d